 ...
 * interface. The network connection is configured and connected upon object
 * creation but the username and token need to be manually set. Afterwards data
 * can be sent to the lighthouse.
 */
public class LighthouseDisplay {
	...
	/**
	 * Returns an existing LighthouseDisplay for the Lighthouse-API which wasn't
	 * closed or creates a new LighthouseDisplay for it.
	 * 
	 * @throws IOException if there is an error while connecting or constructing the
	 *                     web-socket
	 * @throws Exception   if there is an error constructing a web-socket-client
	 */
	...
	/**
	 * Returns an existing LighthouseDisplay for the given URI or Creates a new
	 * LighthouseDisplay with the given settings and connects to the lighthouse
	 * server at the given web-socket address and disables certificate validation if
	 * selfSigned is true. Connection is finalized asynchronous.
	 * 
	 ...
	 *                            web-socket-client
	 */
	public static LighthouseDisplay getDisplay(String destinationURI, boolean allowSelfSigned, int debugOutput)
			throws InvalidAttributeValueException, IOException, Exception {
	...
	/**
	 * Creates a new LighthouseDisplay and sets weather connect and disconnect
	 * messages should be printed in stdOut
	 */
	...
	/**
	 * setter for the configured username. The username may be change during a
	 * connection.
	 * 
	...
	/**
	 * setter for the token to be used. This token may be change during a
	 * connection.
	 * 
	...
	/**
	 * This enables the Key-Inputs from the Lighthouse-Display. The username and
	 * token have to be set before. This will request the Key-Inputs for the current
	 * user or (if not connected) the user at the time of successful connection.
	 * 
	 * @throws IllegalStateException if the connection is closed or username/token
	 *                               was not set
	 */
	public void enableKeyInputs() throws IllegalStateException {
		...
		if (username == null) {
			throw new IllegalStateException(
					"The Username must be set in the LightouseDisplay to be able to send images.");
		}
		if (token == null) {
	...
	/**
	 * Sends an image to the lighthouse server (Width: 28, Height: 14). The data
	 * should be a byte array consisting of 1176 bytes. Each three bytes are the
	 * red, green and blue color values of a window. The windows start at the
	 * top-left corner and are ordered in rows.
	 *
	 * Example for 2x2 windows indexes of red color byte: 0 3 6 9
	 *
	 * @param data The data to send
	 * @throws IllegalStateException    if the username or token wasn't set Or the
	 *                                  connection was Closed by calling 'close()'.
	 * @throws IllegalArgumentException if the image-data is not 1176 Bytes long.
	 * @throws IOException              if some error occurs during sending of the
	 *                                  data.
	 */
	public void sendImage(byte[] data) throws IllegalStateException, IllegalArgumentException, IOException {
		...
		if (data.length != 1176) {
			throw new IllegalArgumentException(
					"The image data must be exactly 1176 Bytes long (not " + data.length + " Bytes)");
		}
	...
	/**
	 * private class for handling the web-socket (has to be public for the api,
	 * which it is registered to)
	 */
	...
	public class LighthouseDisplayHandler {
		...
		public void onConnect(Session session) {
			...
			if (parent.keyInputEnabled)
				requestKeyData();

			// create a Thread to re-send images every 2s if no image was sent since the
			// last check
						...
						if (connected && !dataSentSinceLastCheck && lastPacket != null) {
							synchronized (sendSynchronizer) {
								...
								if (!dataSentSinceLastCheck) {
									try {
										endpoint.sendBytes(
												ByteBuffer.wrap(Arrays.copyOf(lastPacket, lastPacket.length)));
										endpoint.flush();
									} catch (IOException e) {
										e.printStackTrace();
									}
		...
		public void onMessage(byte buf[], int offset, int length) {
				...
				if (rnum == 200) {
					...
					if (reid == -1) {
						...
						for (Value entry : entries) {
							...
							boolean pressed = payl.get(new ImmutableStringValueImpl("dwn")).asBooleanValue()
									.getBoolean();
			...
			} catch (MessageTypeCastException ignored) {
